...
void sendOverUDP() {
   for (int i = 0; i < WINDOW_SIZE; i++) {
      if (window[i].ack == false){
         ...
         n = sendto(clientSocket, &window[i], sizeof(window[i]), MSG_CONFIRM, (const struct sockaddr *) &server, addrlen);
...
void wait_ack(){
   ...
   for (int i = 0; i < acks_to_wait; i++)
   {
      ...
      n = recvfrom(clientSocket, &newAck, sizeof(&newAck), 0,  (struct sockaddr *) &server, &addrlen);
      if (n < 0) 
